...
def vle():
    ...
    with st.form("StudRegAndCourseForm", clear_on_submit=True):
        ...
        sum_click = st.number_input("Enter sum of clicks",min_value=1)
        button = st.form_submit_button("Submit")
...
def student_info():
    ...
    with st.form("StudentInfoForm", clear_on_submit=True):
        ...
        studied_credits = st.number_input("Enter studied credits",min_value=1)
        code_presentation = st.selectbox('Select semester', semester)
        course = st.selectbox('Select code module', code_module)
        sum_click = st.number_input("Enter sum of clicks",min_value=1)
        After_Clicks = st.number_input("Enter sum of after clicks",min_value=1)
        Before_Clicks = st.number_input("Enter sum of before clicks",min_value=1)
